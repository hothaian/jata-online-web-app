...
export const SingleSellPost = ({ match }) => {
    ...
    useEffect(()=>{
        ...
        const fetchASellPost = async () => {
            try {
              const response = await fetch('http://localhost:8080/api/sellposts/'+post_id); 
              if (!response.ok) {
  ...
  return (
    <>{currentPost ?   <div className="card mb-3 h-100">
            ...
            <div className='border-bottom border-1 border-dark'>
                <div className="card-header d-flex justify-content-between align-items-center">
                    <div className="d-flex align-items-center">
                        <img src={currentPost.seller?.profile_pictureUrl} alt="User Profile" className="rounded-circle me-2" style={{ width: '32px', height: '32px' }} />
                ...
                <div className="card-body">
                    ...
                    <div className="d-flex align-items-center">
                        ...
                        <p className="card-text mb-0">Category:  {currentPost.categories.map(category => (
                ...
                <ul className="list-group">
                    {currentPost.comments && currentPost.comments.map((comment) => (
                        <li key={comment.comment_id} className="list-group-item">
                            <img src={comment.user.profile_pictureUrl} alt="User Profile" className="rounded-circle me-2" style={{ width: '32px', height: '32px' }} />
                            <span className="fw-bold">{comment.user.username}:</span> {comment.description}
                        </li>
                    ))}
                </ul> 
